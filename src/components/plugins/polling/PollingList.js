                ...
                {pollings.map((list) => (
                    <TouchableOpacity onPress={()=>{
                        navigation.navigate('PollingDetail', {
                            qstValue: list.qst,
                            qstid: list.id,
                            btn1: list.answer.filter((n,index) => (index == 0)
                            ).map(function(n){return n.name}),
                            btn2: list.answer.filter((n,index) => (index == 1)
                            ).map(function(n){return n.name}),
                            btn3: list.answer.filter((n,index) => (index == 2)
                            ).map(function(n){return n.name}),
                            value1: list.answer.filter((n,index) => (index == 0)
                            ).map(function(n){return n.value}),
                            value2: list.answer.filter((n,index) => (index == 1)
                            ).map(function(n){return n.value}),
                            value3: list.answer.filter((n,index) => (index == 2)
                            ).map(function(n){return n.value})
                        })
                    }}    
                    >
